...
Di client ada menu **Aksi** (tipis di atas, gambar tetap full):
- **Copy dari target** = block teks di layar target dulu, klik menu itu (ngirim Ctrl+C ke target), tunggu ~1 detik, paste di PC sendiri.
- **Paste ke target** = kirim isi clipboard PC sendiri ke target + otomatis Ctrl+V di sana.
- **Fullscreen (F11)** = layar penuh beneran, **Esc** buat keluar.
